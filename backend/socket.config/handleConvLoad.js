...
async function handleConvLoad (user, ws) {
    try{
        ...
        const [rooms] = await db.query('SELECT support_messages.sender_id , support_messages.content, users.fullname FROM support_messages join users on users.id = support_messages.sender_id  WHERE conversation_id IN (?) order by message_id desc limit 1',[filteredRooms]);
