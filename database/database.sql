...
create table admin (id int , admin_rooms JSON , foreign key (id) references users(id));
...
create table reports (id     , report_id int primary key, product_id int, type enum("product" , "delivery"), content varchar(255), foreign key reports(id) references users(id),  foreign key reports(product_id) references products(products_id));
