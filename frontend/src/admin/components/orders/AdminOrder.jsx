            ...
            <div className="order-top  d-flex justify-content-between">
                ...
                <div className="order-end border h-100 d-flex flex-column align-items-end">
                    ...
                    <div className="toggle text-white" style={{ display : toggleDrop ? 'flex' : 'none' , flexDirection : 'column',position : 'relative', top : '10px'}}>
                        <button className="btn btn-primary" onClick={() => setStatus(() => {if(order.status === 'Pending') return 'OnWay'; if(order.status === 'OnWay') return 'Delivered' })} disabled={status === 'Delivered' ? true : false}>Change Status to {order.status === 'Pending' ? 'OnWay' : order.status === 'OnWay' ? 'Delivered' : 'Delivered'}</button>
