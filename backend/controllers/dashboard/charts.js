...
async function charts(req,res){
    try{
        ...
        const [ soldItems ] = await db.query(`select ordered_items.product_id, ordered_items.amount, orders.created_at, products.title, products.category, products.subcategory, products.price, products.sales_price from ordered_items join orders on ordered_items.order_id = orders.order_id join products on products.products_id = ordered_items.product_id order by orders.created_at;`); //selecting sold items to generate charts data for admin dashobard
