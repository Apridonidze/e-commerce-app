const ReportOption = ({ reason, setTargetReason, targetReason, reasonRef }) => { //importing params from parent component (ReportProduct.jsx)
    return(
        <div className={`report-option-container p-4 rounded-3 ${targetReason?.id === reason.id ? 'active' : ''}`} style={{cursor : 'pointer'}} ref={(el) => reasonRef.current[reason.id] = el} key={reason.id} onClick={() => setTargetReason(reason)}>
