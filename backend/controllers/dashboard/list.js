...
async function list(req,res) {
    try{
        ...
        if(onlineAdminList.length === 0) return res.status(200).json({pending,onWay,delivered, onlineAdmins: onlineAdminList , offlineAdmins}); //returning data if there is no online admin right now
        ...
        return res.status(200).json({pending,onWay,delivered, onlineAdmins: admins , offlineAdmins }); //returning data to admin dashobard
