...
async function list(req,res) {
    try{
        ...
        const [orders] = await db.query("select orders.* , ordered_items.product_id, ordered_items.amount, ordered_items.price from orders  join ordered_items on orders.order_id = ordered_items.order_id join address on orders.address = address.id where orders.user_id = ?" , [req.user.userId]); //fetching order items with joins with products table 
