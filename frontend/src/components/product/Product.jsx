        ...
        <div className="product-container d-flex flex-column rounded-1 " style={{cursor: 'pointer'}} key={prod.products_id} >
            ...
            <div className="product-wrapper" onMouseEnter={() => setToggleBtn(true)} onMouseLeave={() => setToggleBtn(() => (toggleMore ? true : false))}>
                ...
                <div className="product-top w-100 d-flex justify-content-center rounded-1 pt-2 " style={{minHeight : '200px'}} onClick={() => {navigator(`/product/${prod.products_id}`); window.location.reload()}}>
                    {<img className="w-auto px-1 h-auto pb-2 mx-auto" src={`data:image/svg+xml;base64,${JSON.parse(prod.images)[0]}`} style={{maxWidth:'100%', maxHeight : '190px', borderRadius : '10px'}}/>}
                </div>
                ...
                <div className="product-footer d-flex align-items-center border-0" >
                    ...
                    {!user ? <></> : isInCart ? 
                        <button className="btn w-100 fw-bold border-0" style={{backgroundColor : '#10b981', color :'white', height : '50px'}} onClick={() => handleDeleteFromCart(prod?.products_id)}><i class="fa-solid fa-cart-shopping text-white me-2"></i> In Cart</button> : 
                        <button className="btn w-100 fw-bold border-0" disabled={prod.amount <= 0 ? true : false} style={{backgroundColor : '#10b981', color :'white', height : '50px'}} onClick={() => setToggleAddToCart({status : true , product : prod})}><i class="fa-solid fa-cart-shopping text-white me-2"></i>Add To Cart</button>
                    }
                </div>
